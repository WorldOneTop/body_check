...
def get_result(value, json, width, img_path):
    ...
    y = data['boxes'][1][1]
    x = data['boxes'][1][0]
    data_list = [] # 근사치 값 특정하기 위해서
    
    # 해당 글자의 측정값을 추측되는 위치(x+, 비슷한y)로 1차 필터링
    for j in json['result']:  # 적절한 숫자 추출
        # 글자의 위치조건이 맞았을경우
        if(j['boxes'][1][1] in range(y-15, y+20)) and (j['boxes'][0][0] > x):
            numbers = re.findall("\d*\.?\d+", j['recognition_words'][0]) # 문자열 속 숫자 추출
            for i in numbers:
                if(3 < float(i) < 160): #3~150사이의 값(체중, 골격근량, 체지방률)
                    data_list.append([float(i), j])
                    
    if(not data_list): # 그래도 없을때
        print('"'+value+'"의 추정되는 측정 값을 찾지못함.')
        return None

    # 후보들 중에 가장 근사치, 여기서부터 data는 원하는 측정값 의미
    # if (value == '체중'):    # 평균치를 75로 두고 가장 근사값 찾기
    #     index = np.abs(np.array(data_list)-75).argmin()
    # else:                   # 평균치를 20으로 두고 가장 근사값 찾기
    #     index = np.abs(np.array(data_list)-20).argmin()
    
    # # 세로의 위치가 가까운 값으로 근사치 정하기  아래는 세로로 1번째인덱스의 배열 추출
    # cache = []
    # for x in np.array(data_list)[:, 1:]:
    #     cache.append(list(x)[0]['boxes'][0][1])
    # index = np.abs(np.array(cache)-data['boxes'][0][1]).argmin()
    
    # 아니면 추정값 박스 왼쪽에 색깔 검출해서 박스가 대충 있는지?? 해보기
    # 만약 추정 값이 2개이상이라면
    ...
    if(1 < len(data_list)):
        ...
        if(not index):
            if (value == '체중'):    # 평균치를 75로 두고 가장 근사값 찾기
                index = np.abs(np.array(data_list)[:,0]-75).argmin()
            else:                   # 평균치를 20으로 두고 가장 근사값 찾기
                index = np.abs(np.array(data_list)[:,0]-20).argmin()
    ...
    if(type(text) == list):
        ...
        for value in text:
            ...
            if(cache):
                result.append(cache)
                print(cache[2])
        return vision.saveImageResult(file_path, file_name+".jpg", result)
    else:
